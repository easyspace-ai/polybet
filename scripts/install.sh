...
#   curl -fsSL https://raw.githubusercontent.com/polybet-ai/polybet/main/scripts/install.sh | bash -s -- --with-server
#
# After installation, run `polybet setup` to configure your environment.
#
set -euo pipefail
...
# Configuration
# ---------------------------------------------------------------------------
REPO_URL="https://github.com/polybet-ai/polybet.git"
REPO_WEB_URL="https://github.com/polybet-ai/polybet"  # without .git, for GitHub web APIs
INSTALL_DIR="${MULTICA_INSTALL_DIR:-$HOME/.polybet/server}"
BREW_PACKAGE="polybet-ai/tap/polybet"
...
install_cli_brew() {
  ...
  if ! brew tap polybet-ai/tap 2>/dev/null; then
  ...
  if ! brew install "$BREW_PACKAGE" 2>/dev/null; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      ...
      fail "Failed to install polybet via Homebrew."
    fi
  else
...
install_cli_binary() {
  ...
  if ! curl -fsSL "$url" -o "$tmp_dir/polybet.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/polybet.tar.gz" -C "$tmp_dir" polybet

  # Try /usr/local/bin first, fall back to ~/.local/bin
  local bin_dir="/usr/local/bin"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/polybet" "$bin_dir/polybet"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/polybet" "$bin_dir/polybet"
  else
    bin_dir="$HOME/.local/bin"
    mkdir -p "$bin_dir"
    mv "$tmp_dir/polybet" "$bin_dir/polybet"
    chmod +x "$bin_dir/polybet"
  ...
  rm -rf "$tmp_dir"
  ok "Multica CLI installed to $bin_dir/polybet"
}
...
install_cli() {
  if command_exists polybet; then
    local current_ver
    # `polybet version` outputs "polybet v0.1.13 (commit: abc1234)" — extract just the version
    current_ver=$(polybet version 2>/dev/null | awk '{print $2}' || echo "unknown")
    ...
    local new_ver
    new_ver=$(polybet version 2>/dev/null | awk '{print $2}' || echo "unknown")
  ...
  # Verify
  if ! command_exists polybet; then
    fail "CLI installed but 'polybet' not found on PATH. You may need to restart your shell."
  fi
}
...
setup_server() {
  ...
  for i in $(seq 1 45); do
    if curl -sf http://localhost:7655/health >/dev/null 2>&1; then
      ready=true
      break
    fi
    sleep 2
...
run_default() {
  ...
  printf "\n"
  printf "     ${CYAN}polybet setup${RESET}                # Connect to Multica Cloud (polybet.ai)\n"
  printf "     ${CYAN}polybet setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/polybet-ai/polybet/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  ...
  printf "  ${BOLD}Backend:${RESET}   http://localhost:7655\n"
  printf "  ${BOLD}Server at:${RESET} %s\n" "$INSTALL_DIR"
  printf "\n"
  printf "  ${BOLD}Next: configure your CLI to connect${RESET}\n"
  printf "\n"
  printf "     ${CYAN}polybet setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/polybet-ai/polybet/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  ...
  if command_exists polybet; then
    polybet daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "After installation, run 'polybet setup' to configure your environment."
        exit 0
        ;;
