...
function fetchPolybetBootstrap(): PolybetProjectBootstrap {
  try {
    const result = ipcRenderer.sendSync(
      "polybet-bootstrap:get",
    ) as PolybetProjectBootstrap | undefined;
  ...
  return {
    project: { ok: false, error: { message: "Polybet bootstrap unavailable" } },
    bundledBinaryPresent: false,
    needsProjectSetup: false,
    needsOutboundVerification: false,
...
const polybetBootstrap = fetchPolybetBootstrap();
...
function fetchSystemLocale(): string {
  const arg = process.argv.find((a) => a.startsWith("--polybet-locale="));
...
const desktopAPI = {
  ...
  runtimeConfig,
  /** Local embedded Go server bootstrap (JSON under ~/.polybet/). */
  polybetBootstrap,
  /** Persist ~/.polybet/polybet-project.json then relaunch via `relaunchApp`. */
  ...
  verifyPolymarketOutbound: (
    ...
    ipcRenderer.invoke("polybet-project:verify-outbound", opts),
