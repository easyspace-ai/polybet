...
describe("runtime config", () => {
  ...
  it("strips the leading api. label when deriving appUrl", () => {
    expect(
      parseRuntimeConfig(
        JSON.stringify({ schemaVersion: 1, apiUrl: "https://api.polybet.ai" }),
      ),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.polybet.ai",
  ...
  it("derives ws for http api URLs", () => {
    expect(deriveWsUrl("http://localhost:7655")).toBe("ws://localhost:7655/ws");
  });
  ...
  it("rejects non-http api schemes", () => {
    expect(() =>
      parseRuntimeConfig(JSON.stringify({ schemaVersion: 1, apiUrl: "file:///tmp/polybet" })),
  ...
  it("preserves electron-vite dev env precedence", () => {
    expect(
      runtimeConfigFromDevEnv({
        apiUrl: "http://dev-api.example.test:7655/",
        wsUrl: "ws://dev-api.example.test:7655/ws/",
  ...
  it("falls back to local web URL when dev apiUrl is localhost", () => {
    expect(runtimeConfigFromDevEnv({ apiUrl: "http://localhost:7655" })).toEqual({
      schemaVersion: 1,
  ...
  it("derives dev appUrl by stripping the leading api. label", () => {
    ...
    expect(
      runtimeConfigFromDevEnv({ apiUrl: "https://api.test.polybet.ai" }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.polybet.ai",
      wsUrl: "wss://api.test.polybet.ai/ws",
      appUrl: "https://test.polybet.ai",
    });
  });
  ...
  it("dev VITE_APP_URL still wins over apiUrl-derived value", () => {
    expect(
      runtimeConfigFromDevEnv({
        apiUrl: "https://api.test.polybet.ai",
        appUrl: "https://staging.polybet.ai",
      }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.polybet.ai",
