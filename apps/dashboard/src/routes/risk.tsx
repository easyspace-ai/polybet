...
import { useRiskControlCache, applyRiskPositionPatch } from "@/hooks/useRiskControlCache";
import {
  postRiskClosePosition,
  postRiskCloseAll,
  patchRiskPosition,
  postRiskOfficialRefresh,
  postRiskHidePosition,
  postRiskTasksClear,
} from "@/lib/api";
...
function bookSubStatusLabel(
  ...
  if (!sub.clientSubscribed) {
    return {
      ...
      title: "本页未向 Risk WS 发送 subscribePolyBook",
    };
  }
  if (!sub.upstreamSubscribed) {
    return {
      text: "上游×",
      className: "text-warning",
      title: "服务端 CLOB 行情 WS 未订阅此 token",
    };
  }
  if (sub.stale) {
    const age =
...
function RiskPage() {
  const { positions, meta, tasks, loading, error, refresh, lastRefresh, polyOrderbookConnected } =
    useRiskControlCache();
  ...
  const applyRiskControls = async (id: string) => {
    ...
    try {
      const resp = await patchRiskPosition(id, { stopLossPct: sl, highWaterCents: hw });
      if (resp.position) {
        applyRiskPositionPatch(resp.position);
        setDrafts((prev) => ({
          ...prev,
                  ...
                  <table className="w-full min-w-[720px] table-fixed text-[11px]">
                    ...
                    <tbody className="divide-y divide-border">
                      {positions.map((p) => {
                        ...
                        const bookSub = bookSubStatusLabel(p.bookSub, polyOrderbookConnected);
