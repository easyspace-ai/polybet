...
import { BetSlip, type BetSlipSelection } from '@/components/bet-slip';
import { MatchDetail } from '@/components/match-detail';
import { LeagueTree, type LeagueNavTag } from '@/components/league-tree';
import { BottomSheet } from '@/components/bottom-sheet';
import { OddsLegend } from '@/components/odds-legend';
...
import { getConfig, type Market, type MarketOutcome } from '@/lib/api';
...
function OddsCell({
  ...
  if (!outcome || !decimal) {
    return (
      <div className={cn(heightClass, 'flex items-center justify-center rounded-md border border-border text-muted-foreground text-sm')}>
        —
      </div>
    );
  }
  const isSelected = selection?.outcomeId === outcome.outcomeId;
  const venueText = 'text-primary';
  const venueHover = 'hover:bg-primary/10';
  const venueSel = 'bg-primary/15 border-primary ring-1 ring-primary/40';
  const explicitLabel =
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isSelected ? venueSel : `bg-surface border-border ${venueHover}`,
      )}
    >
      {explicitLabel !== null ? (
        <span className={cn('font-mono tracking-wider text-muted-foreground leading-none', compact ? 'text-[9px]' : 'text-[10px]')}>{explicitLabel}</span>
      ) : (
...
function SoccerMatchRow({ group, selection, onRowClick, onOddsClick }: MatchRowProps) {
  ...
  return (
    <div
      className="hidden md:grid items-center px-4 py-2.5 border-b border-border hover:bg-surface/60 cursor-pointer transition-colors"
      style={{ gridTemplateColumns: SOCCER_GRID_COLS, columnGap: SOCCER_GRID_GAP }}
      onClick={() => onRowClick(group)}
    >
      <div className="min-w-0">
        <p className="text-[13px] font-semibold text-foreground truncate leading-tight">{group.name.split(' vs ')[0]?.trim() ?? group.name}</p>
        <p className="text-[13px] text-muted-foreground truncate leading-tight">{group.name.split(' vs ')[1]?.trim() ?? ''}</p>
        <p className="font-mono text-[9px] tracking-wider text-muted-foreground mt-1">
      ...
      <div className="text-right">
        {extraCount > 0 && (
          <span className="font-mono text-[10px] text-muted-foreground">+{extraCount}</span>
        )}
      </div>
    </div>
  );
...
function NAMatchRow({ group, selection, onRowClick, onOddsClick }: MatchRowProps) {
  ...
  return (
    ...
    >
      <div className="min-w-0">
        <p className="text-[13px] font-semibold text-foreground truncate leading-tight">{team1 ?? group.name}</p>
        <p className="text-[13px] text-muted-foreground truncate leading-tight">{team2 ?? ''}</p>
        <p className="font-mono text-[9px] tracking-wider text-muted-foreground mt-1">
          {formatKickoff(group.startTime)}
        </p>
      </div>
...
function SoccerMatchCard({ group, selection, onRowClick, onOddsClick }: MatchRowProps) {
  ...
  return (
    <div
      className="px-3 py-2.5 border-b border-border hover:bg-surface/40 cursor-pointer transition-colors"
      // onClick={() => onRowClick(group)}
    >
      <div className="grid grid-cols-[2fr_3fr] gap-2.5 items-start">
        ...
        <div className="flex flex-col gap-1 min-w-0">
          <span className="font-mono text-[9px] uppercase tracking-[0.15em] text-muted-foreground text-center truncate">
            独赢 · 1 / X / 2
          </span>
...
function SoccerColHead() {
  return (
    <div
      className="hidden md:grid items-center px-4 py-1.5 border-b border-border font-mono text-[9px] tracking-[0.18em] text-muted-foreground bg-background"
      style={{ gridTemplateColumns: SOCCER_GRID_COLS, columnGap: SOCCER_GRID_GAP }}
    >
      <span />
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((x) => !x)}
        className="w-full h-11 pl-3 pr-9 flex items-center justify-between rounded-md border border-border bg-surface text-foreground font-mono text-[12px] font-semibold tracking-[0.1em] focus:outline-none focus:border-ring"
      >
        <span className="truncate">{selectedLabel}</span>
        <span
          aria-hidden="true"
          className="absolute right-3 top-1/2 -translate-y-1/2 font-mono text-[10px] text-muted-foreground"
        >
          ▾
        </span>
      </button>
      ...
      {open && (
        <div
          role="listbox"
          aria-label="选择联赛"
          className="absolute left-0 right-0 z-30 mt-1 max-h-[60vh] overflow-y-auto rounded-md border border-border bg-surface shadow-lg"
        >
          {tags.map(({ tag, label, count }) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'w-full px-3 py-2 flex items-center justify-between font-mono text-[12px] tracking-[0.05em] border-b border-border/50 last:border-b-0 hover:bg-sidebar transition-colors',
                  isSelected ? 'bg-primary/10 text-primary font-semibold' : 'text-foreground',
                )}
              >
                <span className="truncate">{label}</span>
                <span className={cn('shrink-0', isSelected ? 'text-primary/70' : 'text-muted-foreground')}>({count})</span>
              </button>
            );
          })}
        </div>
...
function TopStrip({
  ...
  return (
    <div className="h-11 shrink-0 flex items-center gap-4 px-4 bg-sidebar/80 backdrop-blur-md border-b border-border z-10">
      <div className="flex items-center gap-2">
        <span className="relative flex h-[6px] w-[6px]">
          <span className="absolute inline-flex h-full w-full rounded-full bg-primary opacity-60 animate-flicker" />
          <span className="relative inline-flex rounded-full h-[6px] w-[6px] bg-primary glow-primary" />
        </span>
        <span className="font-mono text-[10px] font-semibold tracking-[0.15em] text-muted-foreground uppercase">{title}</span>
      </div>
      <span className="font-mono text-[10px] px-1.5 py-0.5 rounded-md bg-surface border border-border text-muted-foreground">
        {fetchAge}
      </span>
...
function MarketsPage() {
  ...
  if (loading) {
    return (
      <div className="flex h-full items-center justify-center text-muted-foreground font-mono text-xs tracking-widest">
        加载市场中…
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex h-full items-center justify-center p-8">
        <div className="rounded-md border border-down/40 bg-down/10 px-4 py-3 font-mono text-xs text-down">
          {error}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex h-full min-h-0">
      <aside className="hidden md:block w-48 shrink-0 border-r border-border bg-sidebar overflow-y-auto">
        {leagueTree}
      </aside>
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        {selectedGroup ? (
          ...
          <>
            ...
            <div className="md:hidden shrink-0 px-3 py-2 bg-sidebar border-b border-border">
              <LeagueDropdown
                tags={leagueNavTags}
                selectedTag={selectedLeague}
                onSelectTag={handleSelectTag}
            ...
            <div className="flex-1 overflow-y-auto">
              {filteredGroups.length === 0 ? (
                <p className="px-4 py-10 font-mono text-xs text-muted-foreground">
                  {markets.length === 0
                    ? '暂无市场数据 — 同步任务可能仍在运行。'
                ...
                Array.from(groupsByDate.entries()).map(([dateKey, dateGroups]) => {
                  ...
                  return (
                    <div key={dateKey}>
                      <div className="sticky top-0 z-20 px-4 py-2 bg-background border-b border-border flex items-center gap-2.5">
      ...
      <aside className="hidden md:flex w-80 shrink-0 border-l border-border bg-sidebar overflow-hidden flex-col">
        <BetSlip
          selection={selection}
