...
import { isGlobalPath } from "@polybet/core/paths";
...
/**
 * Open a link — internal paths dispatch polybet:navigate, external open new tab.
 *
...
export function openLink(href: string, currentSlug?: string | null): void {
  if (href.startsWith("/")) {
    ...
    window.dispatchEvent(
      new CustomEvent("polybet:navigate", { detail: { path } }),
    );
  } else {
