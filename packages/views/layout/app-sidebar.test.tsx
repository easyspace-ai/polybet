...
import { ApiError } from "@polybet/core/api";
...
vi.mock("@polybet/ui/components/ui/sidebar", () => ({
...
vi.mock("@polybet/ui/components/ui/collapsible", () => ({
...
vi.mock("@polybet/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));
...
vi.mock("@polybet/core/api", async (importOriginal) => ({ ...(await importOriginal<typeof import("@polybet/core/api")>()), api: {} }));
vi.mock("@polybet/core/inbox/queries", () => ({ deduplicateInboxItems: (items: unknown[]) => items, inboxKeys: { list: () => ["inbox"] } }));
vi.mock("@polybet/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@polybet/core/issues/stores/create-mode-store", () => ({ useCreateModeStore: { getState: () => ({ lastMode: "agent" }) } }));
vi.mock("@polybet/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@polybet/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@polybet/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@polybet/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@polybet/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
vi.mock("@polybet/core/runtimes/hooks", () => ({ useMyRuntimesNeedUpdate: () => false }));
